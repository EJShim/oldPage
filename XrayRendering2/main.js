...
function Initialize()
{
  ...
  //init renderrer
  m_renderer.setClearColor(0x111111, 0);
...
function DrawGeometry()
{
  ...
  var material = new THREE.MeshPhongMaterial( { color:'white', shininess:30, specular:0x000000 } );
  detector = new THREE.Mesh( geometry, material );
  detector.receiveShadow = true;

  detector.position.set(pointLight.position.x, 500, pointLight.position.z);
  ...
  var light = new THREE.AmbientLight( 0xffffff, 1.3 ); // soft white light
  m_scene.add( light );



  //Add Second Point light
  var pointLight2 = new THREE.SpotLight( 0xffffff, 1, 10000 );
  pointLight2.intensity = -1;
  console.log(pointLight2);

  pointLight2.castShadow = true;
  pointLight2.shadow.camera.far = 2000;
  pointLight2.shadow.camera.fov = 26;
  pointLight2.shadow.mapSize.width = 5000;
  pointLight2.shadow.mapSize.height = 5000;

  pointLight2.position.y = -150;
  pointLight2.position.x = -1300;
  pointLight2.position.z = -600;

  var shadowCameraHelper2 = new THREE.CameraHelper(pointLight2.shadow.camera);
  shadowCameraHelper2.material.linewidth = 5.0;
  shadowCameraHelper2.material.color = new THREE.Color(0.0, 1.0, 0.0);
  m_scene.add(shadowCameraHelper2);

  // var lightGeometry = new THREE.SphereGeometry( 19, 12, 6 );
  // var lightMaterial = new THREE.MeshBasicMaterial( { color: 'red' } );
  var lightSphere2 = new THREE.Mesh( lightGeometry, lightMaterial );
  pointLight2.add( lightSphere2 );

  m_scene.add(pointLight2);


  //add detector Plane
  var geometry2 = new THREE.BoxGeometry( 5, 500, 900 );
  detector2 = new THREE.Mesh( geometry2, material );

  detector2.receiveShadow = true;
  detector2.position.set(pointLight2.position.x + 2000, pointLight2.position.y, pointLight.position.z);

  pointLight2.target = detector2;

  m_scene.add( detector2 );
